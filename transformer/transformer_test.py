...
class TransAm(nn.Module):
    def __init__(self, feature_size=250, num_layers=1, dropout=0.1):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=feature_size, nhead=25, dropout=dropout)
...
def get_data():
    ...
    #series = pd.read_csv('./4665896876_28_timestamp-cpu.csv', header=None).iloc[:, 1]
    #series = pd.read_csv('./data/1863690462_1_9260_.csv').iloc[:, 4]
    series = pd.read_csv('./data/4665896876_28_9716_.csv').iloc[:, 4]
...
def plot_and_loss(eval_model, data_source, epoch):
    ...
    tune = 'Tune_' if fine_tune else 'noTune_'
    eval_model.eval()
    total_loss = 0.
    ...
    for i in range(plot_spans):
        ...
        try:
            os.makedirs('test/%sgraph/epoch-%d' % (tune, epoch))
        except Exception as e:
            pass
        plt.savefig('test/%sgraph/epoch-%d/%d.png' % (tune, epoch, i))
        plt.close()
...
def finetuneModel():
    for epoch in range(1, epochs + 1):
        ...
        valid(epoch, epoch_start_time)
        
        scheduler.step()

def valid(epoch, epoch_start_time):

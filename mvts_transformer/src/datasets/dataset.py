...
def collate_superv(data, max_len=None):
    ...
    batch_size = len(data)
    features, labels, IDs = zip(*data) # 这里有了序列信息
...
class ClassiregressionDataset(Dataset):
    ...
    def __init__(self, data, indices):
        ...
        self.data = data  # this is a subclass of the BaseData class in data.py
        self.IDs = indices  # list of data IDs, but also mapping between integer index and ID
        self.feature_df = self.data.feature_df.loc[self.IDs]#到这还没有序列信息
        
        self.labels_df = self.data.labels_df.loc[self.IDs]#到这还没有序列信息
        
        #增加序列信息
        self.seq_length = 30
        self.seq_split_feature = []
        self.seq_split_label = []
        for i in range(len(self.feature_df) - self.seq_length):
            self.seq_split_feature.append(self.feature_df.values[i: i + self.seq_length])
            self.seq_split_label.append(self.labels_df.values[i: i + self.seq_length][-1])
    def __getitem__(self, ind):
        """
        For a given integer index, returns the corresponding (seq_length, feat_dim) array and a noise mask of same shape
        Args:
        ...
        #X = self.feature_df.loc[self.IDs[ind]].values  # (seq_length, feat_dim) array 这里有序列信息 原数据集很多行有相同的id，因此一次取出144行
        X = self.seq_split_feature[ind]
        #y = self.labels_df.loc[self.IDs[ind]].values  # (num_labels,) array
        y = self.seq_split_label[ind]

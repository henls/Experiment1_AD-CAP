...
class TSRegressionArchive(BaseData):
    ...
    def load_single(self, filepath):
        ...
        df.to_csv('debug/df.csv', header=None, index=0)
        return df, labels_df
...
class GoogleRegressionData(BaseData):
    """
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        self.feature_names = ['mean cpu usage', 'canonical memory usage']
        self.feature_df = self.all_df[self.feature_names]
    ...
    def load_single(self, filepath):
        ...
        if num_nan > 0:
            logger.warning("{} nan values in {} will be replaced by 0".format(num_nan, filepath))
            df = df.fillna(0)
        labels_df = pd.DataFrame(np.array(df[1:]), columns=df.columns)
        df = df[:-1]
        return df, labels_df

...
public class TransitionTable
{
  ...
  public TransitionTable(int columns, int rows, Stats statistics, boolean UseTPCWThinkTime)
  {
    ...
    //rubis = new RUBiSProperties("rubis"); //add by wxh
    nbColumns = columns;
    nbRows = rows;
    stats = statistics;
    ...
    /*Integer foo = new Integer(rubis.getProperty("workload_number_of_clients_per_node"));
    int nbOfClients = foo.intValue();
    thinkTme = new double[nbOfClients];
    for (int i = 0; i < rubis.getNbOfClients(); i++){
      thinkTme[i] = rand.nextDouble() * 10.0;
    }*/
  }
...
//    System.out.println("Transition matrix successfully build.");
  ...
  private long TPCWthinkTime()
  {
    ...
    /*double TK = thinkTme[(int)Thread.currentThread().getId()];
    double r = rand.nextDouble();
    if (r < (double)4.54e-5)
      return ((long) (r+0.5));
    return  ((long) ((((double)-1 * TK)*Math.log(r))+0.5));*/
  }

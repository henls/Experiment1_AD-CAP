...
class DQN(object):
    def __init__(self):                                                         # 定义DQN的一系列属性
        ...
        self.memory = torch.zeros(MEMORY_CAPACITY, N_STATES * 2 + 2, device=device)
    ...
    def choose_action(self, x):                                                 # 定义动作选择函数 (x为状态)
        ...
        if random.random() < EPSILON:
            ...
            action = torch.max(actions_value, 1)[1].data                        # 输出每一行最大值的索引，并转化为numpy ndarray形式
    ...
    def learn(self):                                                            # 定义学习函数(记忆库已满后便开始学习)
        ...
        #b_s = torch.FloatTensor(b_memory[:, :N_STATES], device=device)
        b_s = b_memory[:, :N_STATES]
        # 将32个s抽出，转为32-bit floating point形式，并存储到b_s中，b_s为32行4列
        #b_a = torch.LongTensor(b_memory[:, N_STATES:N_STATES+1], device=device)
        b_a = b_memory[:, N_STATES:N_STATES+1]
        # 将32个a抽出，转为64-bit integer (signed)形式，并存储到b_a中 (之所以为LongTensor类型，是为了方便后面torch.gather的使用)，b_a为32行1列
        #b_r = torch.FloatTensor(b_memory[:, N_STATES+1:N_STATES+2], device=device)
        b_r = b_memory[:, N_STATES+1:N_STATES+2]
        # 将32个r抽出，转为32-bit floating point形式，并存储到b_s中，b_r为32行1列
        #b_s_ = torch.FloatTensor(b_memory[:, -N_STATES:], device=device)
...
for i in range(400):                                                    # 400个episode循环
    ...
    while True:                                                         # 开始一个episode (每一个循环代表一步)
        ...
        s_, r, done, info = env.step(a.item())                                 # 执行动作，获得反馈
        ...
        new_r = r1 + r2
        new_r = torch.tensor(new_r, device=device)
        ...
        if done:       # 如果done为True
            ...
            print('episode%s---reward_sum: %s' % (i, episode_reward_sum))
            break              
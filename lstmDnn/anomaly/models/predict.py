...
class model():
    ...
    def dataload(self,pth, sequence_length, test_ratio=0.5, train=True):
        ...
        if train:
            for i in range(idx_st, idx_st + int((idx_ed - idx_st) * test_ratio)):
                ...
                if count % self.mb_size == 0:
                    yield np.array(data_mbSize).reshape(-1, sequence_length + 1, data.shape[1])
                    #print(len(data_mbSize))
                    data_mbSize = []
                
                data_mbSize.append(data[i:i + sequence_length + 1])
                count += 1 
            ...
            for i in range(idx_st + int((idx_ed - idx_st) * test_ratio), idx_ed):
                #剩余的全送出去预测
                data_mbSize.append(data[i:i + sequence_length])
            yield np.array(data_mbSize).reshape(-1, sequence_length, data.shape[1])
        

    def trainday(self, is_training, anomaly, f):
        self.pointless = []
        #pth = r'/home/wxh/lstmDnn/anomaly/pred_data/test/4665896876_28.csv'
        pth = r'/home/wxh/lstmDnn/anomaly/pred_data/test/28-normal.csv'
        data = self.dataload(pth, sequence_length = self.sentence)
        ...
        last_seq = 0
        #取一些test数据做验证看效果
        data_test = self.dataload(pth, sequence_length = self.sentence, train=False)
        ...
        if self.train_test:
            ...
            for i in range(self.test_train_epoch):
                ...
                while raw_batch is not None:
                    ...
                    'task':raw_batch[:, :, 3],
                    'machine':raw_batch[:, :, 4],
                    'x': raw_batch[:, :endx, 5:-3].reshape(-1, self.sentence, feature_size),
                    't': raw_batch[:, 1:endt, 5:-3].reshape(-1, self.sentence, feature_size)
                    }
                    if raw_batch is not None:
                        last_seq = data_dict_test['t'][-1].reshape(-1, self.sentence, feature_size)
                ...
                if self.earlyStop(cur_loss) and i>= self.min_epoch:
    ...
    def load(self, jsonfile, outfile, logfile):
        ...
        sentence_length = dataspecs['sentence_length']
        self.sentence = sentence_length
        ...
        self.model = ModelRunner(avg_loss, ph_dict, learnrate=lr, decay_steps=500)
        self.eval_tensors = [avg_loss, line_losses, pred_y]

        files = dataspecs['test_files']
        idx = 0
        self.saver = tf.train.Saver()
        self.test_train_epoch = 6000
        self.idx = idx
        self.train_test = 1

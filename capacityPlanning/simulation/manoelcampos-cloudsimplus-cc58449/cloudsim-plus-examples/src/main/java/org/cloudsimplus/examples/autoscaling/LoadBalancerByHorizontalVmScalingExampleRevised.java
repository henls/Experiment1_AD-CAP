 ...
 */
public class LoadBalancerByHorizontalVmScalingExampleRevised {
    /**
    ...
    */
    ...
    public static void main(String[] args) {
        new LoadBalancerByHorizontalVmScalingExampleRevised();
    }

    /**
     * Default constructor that builds the simulation scenario and starts the simulation.
     */
    private LoadBalancerByHorizontalVmScalingExampleRevised() {
        ...
        */
        ...
        simulation.addOnClockTickListener(this::viewVmUtils);
    ...
    private Datacenter createDatacenter() {
        ...
        /*数据中心采样和打印的间隔，15代表隔15s采集一次vm状态，如果过载则扩展，如果15s内有过载多次，只会动作一次
        如果设置的小，15s会动作多次，可能产生很多scale动作。
        */
    ...
    private void viewVmUtils(final EventInfo info){
        final long time = (long) info.getTime();
        int viewInterval = 1;
        if (time % viewInterval == 0) {
            double cpuUtil = broker0.getVmCreatedList().get(0).getCpuPercentUtilization();
            System.out.println("vm 0: cpu utilization is " + cpuUtil);
        }
    }
}
